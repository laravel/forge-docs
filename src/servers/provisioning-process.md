...
Forge does not automatically update other software such as PHP or MySQL automatically, as doing so could cause your server to suffer downtime if your application's code is not compatible with the upgrade. However, it is possible to [install new versions](/servers/php#multiple-php-versions) and [patch existing versions of PHP](/servers/php#updating-php-between-patch-releases) manually via the Forge UI.
